...
//For the scatter/jitter plots, let's attempt to do bua_umh and MSOA in the same frame.
getData('./data/data_bua_umh.csv')
	.then(arr => {
		let meta = arr.map(d => ({
				code: d.code,
				name: d.name,
			...
			metadata.bua_umh.array = meta;
			metadata.bua_umh.lookup = lookup;
			let indicators = arr.map((d, i) => ({
				...meta[i],
            ...
            ['cluster2_umh'].forEach(key => {
				...
				data.bua_umh.indicators = indicators;
			});
//For the scatter/jitter plots, let's attempt to do bua_umh and MSOA in the same frame.
getData('./data/data_bua_utac.csv')
	.then(arr => {
		let meta = arr.map(d => ({
				code: d.code,
				name: d.name,
			...
			metadata.bua_utac.array = meta;
			metadata.bua_utac.lookup = lookup;
			let indicators = arr.map((d, i) => ({
				...meta[i],
            ...
            ['cluster2_utac'].forEach(key => {
				...
				data.bua_utac.indicators = indicators;
			});
